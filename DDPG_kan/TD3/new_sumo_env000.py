...
class SumoEnv(gym.Env):
    ...
    def __init__(
        ...
        self.sim_max_time = 200
        self.prev_sim_step = -1  # 记录上一时间步的仿真时间
        self.stagnant_step_count = 0  # 记录仿真时间停滞的步数
        self.max_stagnant_steps = 5  # 允许的最大停滞步数
    ...
    def modify_output_prefix(self,new_prefix):
        cfg_file = "../networks/sumoconfig_TD3.sumo.cfg"
    ...
    def save_csv(self, episode):
        base_path = "../networks/output_data/output_td3/state"
        self.label = "state"
    ...
    ##仿真启动##
    def start(self,gui=True, vehicleNum=80, network_cfg="../networks/sumoconfig_TD3.sumo.cfg", network_net="../networks/rouabout.net.xml",  vtype1="rlAnget", vtype2="human"):
        self.gui = gui
    ...
    def applyAction(self, action):
        if not self.action_space.contains(action):
            ...
            traci.vehicle.setSpeed(self.vehicle, v_next)
    ...
    def check_terminated(self):
        ...
        if self.sim_step == self.prev_sim_step:
            self.stagnant_step_count += 1
            if self.stagnant_step_count >= self.max_stagnant_steps:
                print("Simulation time is stagnant.")
                return True
        else:
            self.stagnant_step_count = 0  # 重置停滞计数器
    ...
    def compute_reward(self):
        ...
        R_efficient = 38 * (1 - speed_diff / self.speed_limit)  # 调整系数 
        #舒适度j奖励计算
        alpha = 1
