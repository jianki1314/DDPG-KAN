...
def creat_observation():
    state_space_list = ['speed_x', 'speed_y','acc','front_vehicle_distance','front_vehicle_to_speed','rear_vehicle_distance','rear_vehicle_to_speed','Et']
    state_space_low = np.array([0, 0, -10, 0, -14, 0, -14, -3],dtype=np.float32)
    state_space_high = np.array([14, 14, 10, 50, 14, 50, 14, 3],dtype=np.float32)
    obs = spaces.Box(low=state_space_low,high=state_space_high,shape=(8,),dtype=np.float32)
    return obs
...
class SumoEnv(gym.Env):
    ...
    def modify_output_prefix(self,new_prefix):
        cfg_file = "../networks/sumoconfig_LSTM.sumo.cfg"
    ...
    def save_csv(self, episode):
        base_path = "../networks/output_data/output_lstm/state"
        self.label = "state"
    ...
    ##仿真启动##
    def start(self,gui=True, vehicleNum=80, network_cfg="../networks/sumoconfig_LSTM.sumo.cfg", network_net="../networks/rouabout.net.xml",  vtype1="rlAnget", vtype2="human"):
        self.gui = gui
    ...
    def applyAction(self, action):
        ...
        v_next = max(min(speed_limit, v_current + action * self.step_length), 0)

        # 判断是否需要进入默认跟驰模型
        if v_current <= 0.1 and action <= 0:
            # 如果当前速度非常低且加速度是负的，直接进入跟驰模型，以避免长时间停滞
            traci.vehicle.setSpeed(self.vehicle, speed=-1)
            traci.simulationStep()
            print("########进入Krauss模型跟驰速度#########")
        elif v_next < 0.1:
            # 如果预测下一速度会导致怠速，将其设置为最小速度以保持车辆继续前进
            v_next = 0.5  # 设置一个较小的非零速度，避免怠速
    ...
    def compute_reward(self):
        ...
        # if 0 < ttc < safety_ttc:
        #     R_ttc = 200 * math.log(max( ttc / safety_ttc, 0.1 ))
        # else:
        #     R_ttc = 0
        # R_safe = 10*( R_lc + R_ttc )
        ################################################################
        ...
        R_efficient = 80 * (1 - speed_diff / self.speed_limit)  # 调整系数为 8
        ################################################################

        ######################舒适度奖励计算#############################
        alpha = 1
        # jerk = abs( self.compute_jerk() )
        # print("jerk is ===========>>>",jerk)
        ...
        if delta_distance != 0:
            self.Et = electricity_consumption / delta_distance
        else:
            self.Et = 0
        ...
        if delta_distance > min_distance:
            R_energy = -20 * (electricity_consumption / delta_distance)
        else:
            R_energy = -50
        ################################################################
        ...
        if self.speed_x < idle_speed:  # 低于阈值则进行惩罚
            R_idle_penalty = -120* (idle_speed - self.speed_x)
        else:
            R_idle_penalty = 0
        ################################################################
