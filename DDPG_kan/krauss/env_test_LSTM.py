...
class SumoEnv(gym.Env):
    ...
    def save_csv(self, episode):
        base_path = "../networks/output_data/output_default/lstm_input"
        self.label = "lstm_input"
    ...
    ##仿真启动##
    def start(self,gui=True, vehicleNum=80, network_cfg="../networks/sumoconfig1.sumo.cfg", network_net="../networks/rouabout.net.xml",  vtype1="rlAnget", vtype2="human"):
        self.gui = gui
    ...
    def update_params(self):
        ...
        if leader:
            ...
            if leader_id:
                ...
                self.get_safe_info(leader_distance, self.speed_x) ##获取碰撞信息次数
        else:
    ...
    def get_system_info(self):
        ...
        acc = self.acc
        front_vehicle_distance = self.front_vehicle_distacne
        front_vehicle_to_speed = self.front_vehicle_to_speed
        rear_vehicle_distance = self.rear_vehicle_distance
        rear_vehicle_to_speed = self.rear_vehicle_to_speed
        ...
        if delta_distance != 0:
            diff_et = electricity_consumption / delta_distance
        else:
            diff_et = 0
        ...
        state = np.array([
            ...
            round(speed_y, 3),
            round(acc, 3),
            round(front_vehicle_distance, 3),
            round(front_vehicle_to_speed, 3),
            round(rear_vehicle_distance, 3),
            round(rear_vehicle_to_speed, 3),
            round(diff_et,3)
        ])
        # 归一化处理
        # normalized_state = 2 * ((state - state_space_low) / (state_space_high - state_space_low)) - 1
        ...
        state_info = {
            ...
            "speed_y": state[1],
            "acc": state[2],
            "front_vehicle_distance": state[3],
            "front_vehicle_to_speed": state[4],
            "rear_vehicle_distance": state[5],
            "rear_vehicle_to_speed": state[6],
            "Et":state[7]
        }
        return state_info
    ...
    def compute_reward(self):
        ...
        R_efficient = 40 * (1 - speed_diff / self.speed_limit)  # 调整系数为 8
        ################################################################
        ...
        R_arrive = 50 if arrive_flash else 0 
        ################################################################
