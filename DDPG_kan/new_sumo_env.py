...
class SumoEnv(gym.Env):
    ...
    ##仿真启动##
    def start(self,gui=True, vehicleNum=36, network_cfg="networks/sumoconfig_KAN.sumo.cfg", network_net="networks/rouabout.net.xml",  vtype1="rlAnget", vtype2="human"):
        self.gui = gui
        ...
        sumoCmd = [sumoBinary, "-c", self.sumocfg,"--lateral-resolution","3.2",
         "--start", "False", "--quit-on-end", "True","--no-warnings","True", "--no-step-log", "True", "--step-method.ballistic", "true"]
        traci.start(sumoCmd)
    ...
    def generate_vehicles(self, vehicleNum):
        ...
        for i in range(vehicleNum):
            ...
            # depart_speed = str(random.uniform(0, 13.89))
            # arrival_speed = str(random.uniform(0, 13.89))
            # depart_time = str(random.uniform(0, 30))  # 在前30秒内随机发车
            traci.vehicle.add(veh_name, routeID=route_id, typeID=self.humanvType, departLane='free', departSpeed='random')
    ...
    def compute_reward(self):
        ...
        if E_consumed >= E_regenerated and E_consumed != 0:
