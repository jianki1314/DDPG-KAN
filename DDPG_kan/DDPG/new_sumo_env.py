...
class SumoEnv(gym.Env):
    ...
    ##仿真启动##
    def start(self,gui=True, vehicleNum=120, network_cfg="../networks/sumoconfig_KAN.sumo.cfg", network_net="../networks/rouabout.net.xml",  vtype1="rlAnget", vtype2="human"):
        self.gui = gui
    ...
    def update_params(self):
        ...
        if leader:
            ...
            leader_distance = min(leader_distance, 50)  # 限制最大距离为50米
            if leader_id:
        ...
        if follower:
            ...
            follower_distance = min(follower_distance, 50)  # 限制最大距离为50米
            if follower_id:
                follower_speed = traci.vehicle.getSpeed(follower_id)
                self.rear_vehicle_distance = follower_distance
                self.rear_vehicle_to_speed = follower_speed - self.speed_x
        else:
    ...
    def compute_reward(self):
        ...
        R_efficient = 47.5* (1 - speed_diff / self.speed_limit)  # 调整系数

        # speed_diff = abs(self.speed_x - self.speed_limit)
        # # 使用高斯函数形式，避免速度差距大的时候奖励值过低
        # sigma = 3.0  # 标准差决定高斯奖励的分布宽度
        # R_efficient = 30 * np.exp(-0.5 * (speed_diff / sigma) ** 2)
        ...
        R_total = 0.2 * R_safe + 0.30* R_efficient + 0.2 * R_comfort + 0.3 * R_energy + R_arrive + R_idle_penalty
        # print("++++++++++++++++++++++++++++++++++++++++++++++++")
