...
class SumoEnv(gym.Env):
    ...
    def modify_output_prefix(self,new_prefix):
        cfg_file = "../networks/sumoconfig_KAN.sumo.cfg"
    ...
    def save_csv(self, episode):
        base_path = "../networks/output_data/output_kan/state"
        self.label = "state"
    ...
    ##仿真启动##
    def start(self,gui=True, vehicleNum=80, network_cfg="../networks/sumoconfig_KAN.sumo.cfg", network_net="../networks/rouabout.net.xml",  vtype1="rlAnget", vtype2="human"):
        self.gui = gui
...
# 计算子奖励函数
    ...
    def compute_efficiency_reward(self):
        speed_diff = abs(self.speed_limit - self.speed_x)
        R_efficient = 47.5 * (1 - speed_diff / self.speed_limit)
        return R_efficient

    def compute_comfort_reward(self):
        alpha = 1
    ...
    def compute_energy_reward(self):
        ...
        if delta_distance != 0:
            R_energy = -15 * (electricity_consumption / delta_distance)
        else:
            R_energy = -50
        return R_energy

    def compute_arrival_reward(self):
    ...
    def compute_idle_penalty(self):
        idle_speed = 0.1
        if self.speed_x < idle_speed:
            R_idle_penalty = -120 * (idle_speed - self.speed_x)
        else:
            R_idle_penalty = 0
        return R_idle_penalty

    def compute_reward(self):
        R_safe = self.compute_safety_reward()
        R_efficient = self.compute_efficiency_reward()
        R_comfort = self.compute_comfort_reward()
        R_energy = self.compute_energy_reward()
        R_arrive = self.compute_arrival_reward()
        R_idle_penalty = self.compute_idle_penalty()
        ...
        R_total = (weights["safety"] * R_safe +
                   weights["efficiency"] * R_efficient +
                   weights["comfort"] * R_comfort +
                   weights["energy"] * R_energy +
                   R_arrive + R_idle_penalty)
