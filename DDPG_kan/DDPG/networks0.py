...
class ActorNetwork(nn.Module):
    def __init__(
        ...
        width=[observation_dim] + self.net_arch + [action_dim]
        ...
        #     )
        # 原始kan
        self.mu = KAN(width=width,grid=5,k=3,seed=0).to(device)
        # self.mu = KAN.loadckpt('../KAN_data/KAN_model/mark').to(device) # 专家网络
    ...
    def forward(self, obs: th.Tensor) -> th.Tensor:
        raw_action = self.mu(obs) ###动作空间缩放处理##
        min_val = th.FloatTensor(self.action_space.low).to(obs.device)
        max_val = th.FloatTensor(self.action_space.high).to(obs.device)
...
class CriticNetwork(nn.Module):
    def __init__(
        ...
        width = [observation_dim + action_dim] + self.net_arch + [1]
        # # 创建单个 Critic 网络
        ...
        # )
        # 使用 原始KAN 网络
        self.q_network = KAN(width=width,grid=5,k=3,seed=0).to(device)
        self.optimizer = optim.Adam(self.parameters(), lr=beta) 
        # 初始化权重
        self.apply(weight_init)
        self.to(device)
