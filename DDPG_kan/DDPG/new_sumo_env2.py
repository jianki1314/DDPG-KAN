...
class SumoEnv(gym.Env):
    ...
    def get_state(self):
        ...
        if self.vehicle not in traci.vehicle.getIDList():
            ...
            # normalized_state = (state - state_space_low) / (state_space_high - state_space_low)
    ...
    def reward(self):
        ...
        try:
            ...
            # 4. 计算加权总奖励
            R_total = sum(weights[k] * normalized_rewards[k] for k in weights.keys())


            # 6. 完整的断言检查
            assert not np.isnan(R_total), "Total reward is NaN"
            assert not np.isinf(R_total), "Total reward is infinite"
            assert isinstance(R_total, (int, float)), f"Invalid reward type: {type(R_total)}"
