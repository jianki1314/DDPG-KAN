...
class SumoEnv(gym.Env):
    ...
    def colse_step(self):
        ...
        reward = 0.0
        next_state = np.zeros(self.observation_space.shape)
        return (next_state, reward, done, done1, info)
         
    ##对输出文件更名##
    def modify_output_prefix(self,new_prefix):
        cfg_file = "networks/sumoconfig_PPO.sumo.cfg"
    ...
    ##仿真启动##
    def start(self,gui=True, vehicleNum=80, network_cfg="networks/sumoconfig_PPO.sumo.cfg", network_net="../networks/rouabout.net.xml",  vtype1="rlAnget", vtype2="human"):
        self.gui = gui
    ...
    def compute_reward(self):
        ...
        R_efficient = 10 * (self.speed_x / self.speed_limit)
        ...
        if E_consumed != 0:
            R_energy = 1 - ( (E_consumed - E_regenerated) / E_consumed) 
        else:
            R_energy = -1
        
        R_total = 2*R_safe + 3*R_efficient + 2*(R_comfort + R_acc_stability) + 3*R_energy + R_speed_penalty
        return R_total
